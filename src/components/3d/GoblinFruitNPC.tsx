...
interface GoblinFruitNPCProps {
  position?: [number, number, number];
  rotation?: [number, number, number];
  scale?: number;
}

export function GoblinFruitNPC({
  position = [0, 0, 0],
  rotation = [0, 0, 0],
  scale = 0.58
}: GoblinFruitNPCProps) {
  ...
  useNpcRegistry(npcId, 'GoblinFruitNPC', containerRef, stateRef, 'MARKET');
  ...
  useFrame((state, delta) => {
    ...
    if (distToPlayer < 3.5 && stateRef.current !== 'INTERACTING' && !activeDialogNpcId && !hasTalkedToPlayer.current) {
      ...
      setActiveDialog('goblin_fruit_1', npcId);
    }
    ...
    if (stateRef.current === 'INTERACTING') {
      ...
      if (playerPos && distToPlayer > 5) {
         if (activeDialogId && activeDialogId.startsWith('goblin')) {
      ...
      if (barkTimer.current <= 0) {
        ...
        if (!store.npcSpeechBubbles[npcId]) {
           const barks = ["Apples, oranges, fresh fruit!", "Sweetest fruit in the market!", "Need a snack?"];
