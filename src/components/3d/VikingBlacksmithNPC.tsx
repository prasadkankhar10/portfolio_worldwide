...
interface VikingBlacksmithNPCProps {
  position?: [number, number, number];
  rotation?: [number, number, number];
  scale?: number;
}

export function VikingBlacksmithNPC({
  position = [0, 0, 0],
  rotation = [0, 0, 0],
  scale = 0.58
}: VikingBlacksmithNPCProps) {
  const containerRef = useRef<THREE.Group>(null!);
  const { scene, animations } = useGLTF('./models/NPCs/VikingHelmet.glb');
  ...
  useNpcRegistry(npcId, 'VikingBlacksmithNPC', containerRef, stateRef, 'MARKET');
  ...
  useFrame((state, delta) => {
    ...
    if (distToPlayer < 3.5 && stateRef.current !== 'INTERACTING' && !activeDialogNpcId && !hasTalkedToPlayer.current) {
      ...
      setActiveDialog('blacksmith_talk_1', npcId);
    }
    ...
    if (stateRef.current === 'INTERACTING') {
      ...
      if (playerPos && distToPlayer > 5) {
         if (activeDialogId && activeDialogId.startsWith('blacksmith')) {
      ...
      if (barkTimer.current <= 0) {
        ...
        if (!store.npcSpeechBubbles[npcId]) {
           const barks = ["Finest armor here!", "Swords, shields, all forged strong!", "Looking to protect yourself?"];
