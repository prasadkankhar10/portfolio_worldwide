...
export const WizardNPC = ({ 
  ...
  const [activeSpell, setActiveSpell] = useState({ type: 'arcane' as any, color: '#9d4edd', duration: 3.0, scaleMultiplier: 1.0 });
  ...
  useNpcRegistry(npcId, 'WIZARD', containerRef, stateRef, 'TOWN');
    ...
    if (stateRef.current === 'THINKING' && !targetPosRef.current) {
        ...
        if (idleTimer.current > (3.0 + Math.random() * 5.0)) {
           if (Math.random() < 0.4) {
              ...
              idleTimer.current = 0;
              if (Math.random() < 0.1) {
                 setActiveSpell({ type: 'ultimate_arcane', color: '#ff00ff', duration: 6.0, scaleMultiplier: 3.0 });
              } else {
                 const spells = [
                    { type: 'arcane', color: '#9d4edd' },
                    { type: 'fire', color: '#ff2200' },
                    { type: 'void', color: '#330066' },
                    { type: 'water', color: '#00aaff' }
                 ];
