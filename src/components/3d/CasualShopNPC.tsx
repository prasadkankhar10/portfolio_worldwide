...
interface CasualShopNPCProps {
  position?: [number, number, number];
  rotation?: [number, number, number];
  scale?: number;
}

export function CasualShopNPC({
  position = [0, 0, 0],
  rotation = [0, 0, 0],
  scale = 0.58
}: CasualShopNPCProps) {
  ...
  useNpcRegistry(npcId, 'CasualShopNPC', containerRef, stateRef, 'MARKET');
  ...
  useFrame((state, delta) => {
    ...
    if (distToPlayer < 3.5 && stateRef.current !== 'INTERACTING' && !activeDialogNpcId && !hasTalkedToPlayer.current) {
      ...
      setActiveDialog('casual_shop_1', npcId);
    }
    ...
    if (stateRef.current === 'INTERACTING') {
      ...
      if (playerPos && distToPlayer > 5) {
         if (activeDialogId && activeDialogId.startsWith('casual')) {
      ...
      if (barkTimer.current <= 0) {
        ...
        if (!store.npcSpeechBubbles[npcId]) {
           const barks = ["Quality shields for sale!", "Never go to battle unprotected!", "Sturdy and reliable!"];
